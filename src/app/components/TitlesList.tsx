...
export function TitlesList({ municipalityId, municipalityName, onBack }: TitlesListProps) {
  ...
  const [filterType, setFilterType] = useState<'all' | 'TCT-CLOA' | 'TCT-EP'>('all');
  const [filterStatus, setFilterStatus] = useState<'all' | 'Pending' | 'Processed' | 'Released'>('all');
  ...
  const tctCloaCount = titles.filter(t => t.titleType === 'TCT-CLOA').length;
  const tctEpCount = titles.filter(t => t.titleType === 'TCT-EP').length;
  const processedCount = titles.filter(t => t.status === 'Released').length;
    ...
    <div className="flex-1 overflow-auto">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-emerald-800">Land Titles - {municipalityName}</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            ...
            <p className="text-2xl text-emerald-600">{titles.length}</p>
          </div>
          <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200 shadow-sm">
            <p className="text-sm text-emerald-800 mb-1">TCT-CLOA</p>
            <p className="text-2xl text-emerald-700">{tctCloaCount}</p>
          </div>
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 shadow-sm">
            <p className="text-sm text-blue-800 mb-1">TCT-EP</p>
            <p className="text-2xl text-blue-700">{tctEpCount}</p>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            <p className="text-sm text-gray-600 mb-1">Released</p>
            <p className="text-2xl text-emerald-600">{processedCount}</p>
          </div>
        </div>
