  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow h-full flex flex-col">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-medium text-gray-900 truncate">{municipality.name}</h3>
            </div>
          ...
          <div className="bg-emerald-50 p-3 rounded border border-emerald-200">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-emerald-900 font-medium">CLOA</span>
              </div>
              <span className="text-xs text-emerald-700 font-medium">{tctCloaPercentage.toFixed(0)}%</span>
            </div>
            <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
              <span>Processed: {municipality.tctCloaProcessed}</span>
          ...
          <div className="bg-blue-50 p-3 rounded border border-blue-200">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-blue-900 font-medium">EP</span>
              </div>
              <span className="text-xs text-blue-700 font-medium">{tctEpPercentage.toFixed(0)}%</span>
            </div>
            <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
              <span>Processed: {municipality.tctEpProcessed}</span>
      ...
      <div className="mt-auto p-4 pt-0">
        <button
          onClick={() => onViewTitles(municipality.id, municipality.name)}
          className="w-full py-2 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-lg hover:from-emerald-700 hover:to-emerald-800 transition-colors text-sm"
        >
          View Details
        </button>
      </div>
