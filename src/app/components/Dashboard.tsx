...
import { Plus, LogOut, Search, BarChart3, FileText, Building2, CheckCircle, MapPin, Users, Map, AlertCircle, Package, Calendar } from 'lucide-react';
...
export function Dashboard({ username, onLogout, onViewTitles }: DashboardProps) {
  ...
  const totalTctCloa = municipalities.reduce((sum, m) => sum + m.tctCloaTotal, 0);
  const processedTctCloa = municipalities.reduce((sum, m) => sum + m.tctCloaProcessed, 0);
  const totalTctEp = municipalities.reduce((sum, m) => sum + m.tctEpTotal, 0);
  const processedTctEp = municipalities.reduce((sum, m) => sum + m.tctEpProcessed, 0);
  ...
  const pendingTitles = titlesData.filter(t => t.status === 'Pending').length;
  ...
  const releasedTitles = titlesData.filter(t => t.status === 'Released').length;
  const processingTitles = titlesData.filter(t => t.status === 'Processed').length; // Using 'Processed' as processing status
  const pendingDocuments = titlesData.filter(t => t.status === 'Pending').length;
  ...
  const generateTimelineData = () => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const cloaThisMonth = titlesThisMonth.filter(t => t.titleType === 'TCT-CLOA').length;
      const epThisMonth = titlesThisMonth.filter(t => t.titleType === 'TCT-EP').length;

      months.push({
        month: monthName,
        'TCT-CLOA': cloaThisMonth,
        'TCT-EP': epThisMonth
      });
    }
  ...
  const chartData = filteredForChart.map(m => ({
    name: m.name,
    'TCT-CLOA Pending': m.tctCloaTotal - m.tctCloaProcessed,
    'TCT-EP Pending': m.tctEpTotal - m.tctEpProcessed,
  }))
  .filter(item => (item['TCT-CLOA Pending'] || 0) > 0 || (item['TCT-EP Pending'] || 0) > 0) // Only show if there's pending items
  .sort((a, b) => {
    const aTotal = (a['TCT-CLOA Pending'] || 0) + (a['TCT-EP Pending'] || 0);
    const bTotal = (b['TCT-CLOA Pending'] || 0) + (b['TCT-EP Pending'] || 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <header className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white shadow-lg sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-5 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl">Department of Agrarian Reform</h1>
                <p className="text-sm text-emerald-100">Provincial Office - La Union | Land Title Tracking System</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={onLogout}
                className="flex items-center gap-2 px-4 py-2 bg-white text-emerald-700 rounded-lg hover:bg-emerald-50 transition-colors shadow-md"
              >
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-4 rounded-lg border border-emerald-200 shadow-sm">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-xs text-emerald-800">CLOA</p>
                <p className="text-lg font-semibold text-emerald-700">{processedTctCloa} / {totalTctCloa}</p>
                <p className="text-xs text-emerald-600">{totalTctCloa > 0 ? ((processedTctCloa / totalTctCloa) * 100).toFixed(0) : 0}%</p>
              </div>
            </div>
          </div>
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg border border-blue-200 shadow-sm">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-xs text-blue-800">EP</p>
                <p className="text-lg font-semibold text-blue-700">{processedTctEp} / {totalTctEp}</p>
                <p className="text-xs text-blue-600">{totalTctEp > 0 ? ((processedTctEp / totalTctEp) * 100).toFixed(0) : 0}%</p>
              </div>
            </div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-emerald-600">{completedProgress}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-gradient-to-br from-rose-50 to-rose-100 p-4 rounded-lg border border-rose-200 shadow-sm">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-xs text-rose-800">Pending</p>
                <p className="text-xl font-semibold text-rose-700">{pendingTitles}</p>
                <p className="text-xs text-rose-600">Attention</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2>Pending Land Titles by Municipality</h2>
          </div>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="TCT-CLOA Pending" fill="#059669" name="TCT-CLOA Pending" />
              <Bar dataKey="TCT-EP Pending" fill="#2563eb" name="TCT-EP Pending" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2>Monthly Title Processing</h2>
            </div>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={timelineData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="TCT-CLOA" stroke="#059669" activeDot={{ r: 8 }} />
                <Line type="monotone" dataKey="TCT-EP" stroke="#2563eb" />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={[
                {
                  district: 'District 1',
                  'TCT-CLOA': municipalities.filter(m => m.district === 1).reduce((sum, m) => sum + m.tctCloaTotal, 0),
                  'TCT-EP': municipalities.filter(m => m.district === 1).reduce((sum, m) => sum + m.tctEpTotal, 0),
                },
                {
                  district: 'District 2',
                  'TCT-CLOA': municipalities.filter(m => m.district === 2).reduce((sum, m) => sum + m.tctCloaTotal, 0),
                  'TCT-EP': municipalities.filter(m => m.district === 2).reduce((sum, m) => sum + m.tctEpTotal, 0),
                }
              ]}
            >
              <CartesianGrid strokeDasharray="3 3" />
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="TCT-CLOA" fill="#059669" name="TCT-CLOA" />
              <Bar dataKey="TCT-EP" fill="#2563eb" name="TCT-EP" />
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="space-y-3">
                <button
                  className="w-full text-left px-4 py-3 bg-gradient-to-r from-emerald-50 to-emerald-100 hover:from-emerald-100 hover:to-emerald-200 rounded-lg border border-emerald-200 transition-colors"
                  onClick={() => setShowAddTitleDialog(true)}
                >
                  <div className="font-medium text-emerald-800">Add New Title</div>
                  <div className="text-xs text-emerald-600">Create a new land title record</div>
                </button>
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <button className="w-full mt-2 text-center text-sm text-emerald-600 hover:text-emerald-800 font-medium">
                  View All Activity
                </button>
              </div>
            </div>
